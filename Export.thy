...
function (domintros) fac_16 where "fac_16 s cont_27 = ((\<lambda>cont s. cont (s(12 \<mapsto> Any_12 core_iter_ops_Range_A__Iterator_next)));
...
(\<lambda>cont s. case (case s 11 of Some (Any_11 x) => x) of core_option_Option_None  => ((\<lambda>cont. cont_27)) cont s | core_option_Option_Some _ => ((\<lambda>cont s. cont (s(4 \<mapsto> Any_4 (case (case s 11 of Some (Any_11 x) => x) of core_option_Option_Some x => x))));
...
id) cont s)) (\<lambda>s. fac_16 s cont_27) s"
by auto
...
(\<lambda>cont s. fac_16 s (((\<lambda>cont s. cont (s(13 := None)));
(\<lambda>cont s. cont (s(14 := None)));
...
id) cont))) (\<lambda>s. (case s 19 of Some (Any_19 x) => x)) (Map.empty::state)"
