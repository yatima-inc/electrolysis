...
let' t2 ← (d);
let' t3 ← ((t2) =ᵇ ((0 : nat)));
do «$tmp0» ← checked.div usize.bits (t1) (t2);
let' t0 ← «$tmp0»;
let' t5 ← (x);
let' t6 ← (d);
let' t7 ← ((t6) =ᵇ ((0 : nat)));
do «$tmp0» ← checked.rem usize.bits (t5) (t6);
let' t4 ← «$tmp0»;
let' ret ← (((t0), (t4)));
return (ret)
...
let' t2 ← «$tmp0»;
do «$tmp0» ← sem.map (λx, (x, tt)) (checked.add usize.bits (blocks) (t2));
let' t5 ← «$tmp0»;
let' blocks ← ((t5).1);
let' t7 ← (blocks);
...
let' t8 ← (b);
let' t10 ← (i);
do «$tmp0» ← sem.map (λx, (x, tt)) (checked.shl u32.bits ((1 : nat)) (t10));
let' t11 ← «$tmp0»;
let' t9 ← ((t11).1);
...
let' i ← ((t8).2);
let' t11 ← (i);
do «$tmp0» ← sem.map (λx, (x, tt)) (checked.shl u32.bits ((1 : nat)) (t11));
let' t12 ← «$tmp0»;
let' t10 ← ((t12).1);
