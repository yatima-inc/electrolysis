...
dostep tmp__ ← @slice._T_.ops_Index_ops_Range_usize__.index _ t1 t2;
let' t0 ← tmp__;
let' ret ← t0;
return (ret)
...
dostep tmp__ ← @slice._T_.ops_Index_ops_RangeTo_usize__.index _ t3 t4;
let' t2 ← tmp__;
let' t1 ← t2;
let' t0 ← t1;
let' t10 ← self;
...
dostep tmp__ ← @slice._T_.ops_Index_ops_RangeFrom_usize__.index _ t10 t11;
let' t9 ← tmp__;
let' t8 ← t9;
let' t7 ← t8;
let' ret ← (t0, t7);
...
let' tail ← t2.2;
let' t10 ← tail;
dostep tmp__ ← @slice.SliceExt.is_empty _ _ (slice._T_.slice_SliceExt T) t10;
let' t9 ← tmp__;
if t9 then
...
| cmp.Ordering.Less :=
let' t21 ← head;
dostep tmp__ ← @slice._T_.slice_SliceExt.len _ t21;
let' t20 ← tmp__;
...
dostep tmp__ ← @slice._T_.ops_Index_ops_RangeFrom_usize__.index _ t26 t27;
let' t25 ← tmp__;
let' t24 ← t25;
let' s ← t24;
return (inl (f, base, s))
...
let' t32 ← head;
dostep tmp__ ← @slice._T_.slice_SliceExt.len _ t32;
let' t31 ← tmp__;
...
let' ret ← tmp__;
return ret);
dostep tmp__ ← @slice._T_.slice_SliceExt.binary_search_by _ _ fn t0 t1;
let' ret ← tmp__;
return (ret)
