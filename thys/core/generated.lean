...
definition slice._T_.ops_Index_ops_RangeTo_usize__.index {T : Type₁} (self : (slice T)) (index : (ops.RangeTo usize)) : sem ((slice T)) :=
let' self ← self;
let' index ← index;
let' t1 ← self;
let' t4 ← (0 : nat);
...
dostep tmp__ ← @slice._T_.ops_Index_ops_Range_usize__.index _ t1 t3;
let' t0 ← tmp__;
let' ret ← t0;
return (ret)


definition slice._T_.ops_Index_ops_RangeFrom_usize__.index {T : Type₁} (self : (slice T)) (index : (ops.RangeFrom usize)) : sem ((slice T)) :=
let' self ← self;
let' index ← index;
let' t1 ← self;
...
let' t4 ← t5;
let' t7 ← self;
dostep tmp__ ← @slice._T_.slice_SliceExt.len _ t7;
let' t6 ← tmp__;
...
definition slice._T_.slice_SliceExt.split_at {T : Type₁} (self : (slice T)) (mid : usize) : sem (((slice T) × (slice T))) :=
let' self ← self;
let' mid ← mid;
let' t3 ← self;
let' t7 ← mid;
...
dostep tmp__ ← @slice._T_.ops_Index_ops_RangeTo_usize__.index _ t3 t5;
let' t2 ← tmp__;
let' t1 ← t2;
let' t0 ← t1;
let' t11 ← self;
...
dostep tmp__ ← @slice._T_.ops_Index_ops_RangeFrom_usize__.index _ t11 t12;
let' t10 ← tmp__;
let' t9 ← t10;
let' t8 ← t9;
let' ret ← (t0, t8);
...
definition slice._T_.slice_SliceExt.binary_search_by.loop_4 (state__ : F × usize × (slice T)) : sem (sum (F × usize × (slice T)) ((result.Result usize usize))) :=
match state__ with (f, base, s) :=
let' t3 ← s;
let' t6 ← s;
dostep tmp__ ← @slice._T_.slice_SliceExt.len _ t6;
let' t5 ← tmp__;
do tmp__ ← checked.shr t5 (1 : int);
let' t4 ← tmp__;
dostep tmp__ ← @slice._T_.slice_SliceExt.split_at _ t3 t4;
let' t2 ← tmp__;
let' head ← t2.1;
let' tail ← t2.2;
let' t9 ← tail;
dostep tmp__ ← @slice.SliceExt.is_empty _ _ (slice._T_.slice_SliceExt T) t9;
let' t8 ← tmp__;
if t8 then
...
let' t14 ← t15;
let' t13 ← (t14);
dostep tmp__ ← @ops.FnMut.call_mut _ _ _ ops_FnMut__T__F f t13;
match tmp__ with (t11, f) :=
match t11 with
| cmp.Ordering.Less :=
let' t23 ← head;
dostep tmp__ ← @slice._T_.slice_SliceExt.len _ t23;
let' t22 ← tmp__;
let' t21 ← t22 + (1 : nat);
let' base ← base + t21;
let' t28 ← tail;
...
dostep tmp__ ← @slice._T_.ops_Index_ops_RangeFrom_usize__.index _ t28 t29;
let' t27 ← tmp__;
let' t26 ← t27;
let' t25 ← t26;
let' s ← t25;
...
let' t35 ← head;
dostep tmp__ ← @slice._T_.slice_SliceExt.len _ t35;
let' t34 ← tmp__;
let' t32 ← t33 + t34;
...
definition slice._T_.slice_SliceExt.binary_search_by : sem ((result.Result usize usize)) :=
let' self ← self;
let' f ← f;
let' base ← (0 : nat);
let' t1 ← self;
...
definition slice._T_.slice_SliceExt.binary_search {T : Type₁} [cmp_Ord_T : cmp.Ord T] (self : (slice T)) (x : T) : sem ((result.Result usize usize)) :=
let' self ← self;
let' x ← x;
let' t0 ← self;
let' t3 ← x;
...
let' t0 ← p;
let' t2 ← t3;
dostep tmp__ ← @cmp.Ord.cmp _ cmp_Ord_T t0 t2;
let' ret ← tmp__;
return ret);
dostep tmp__ ← @slice._T_.slice_SliceExt.binary_search_by _ _ fn t0 t2;
let' ret ← tmp__;
return (ret)
